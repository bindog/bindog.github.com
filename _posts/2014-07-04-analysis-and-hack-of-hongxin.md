...
![大酒神从零Rap之我不能忍](http://bindog.qiniudn.com/hack-hongxing/dota.png)
...
![红杏主界面](http://bindog.qiniudn.com/hack-hongxing/hongxin-not-vip.png)

红杏的具体使用方法官网已经讲得很详细了，由于我是非VIP用户，因此不能添加自己的科学上网列表，不过谷歌学术已经可以正常访问了~

![熟悉的谷歌学术又回来了](http://bindog.qiniudn.com/hack-hongxing/scholar-google.png)

#0x02 红杏的原理

那么红杏到底是怎么工作的呢，它的原理是什么呢？下面我们先通过`Wireshark`一探红杏背后的秘密。启动`Wireshark`进行抓包，然后用`Chrome`访问Google，在`Wireshark`中得到如下结果

![Wireshark抓包结果](http://bindog.qiniudn.com/hack-hongxing/wireshark.png)
...
![红杏的登录界面](http://bindog.qiniudn.com/hack-hongxing/hongxin-login-page.png)

然后按F12打开开发人员工具，切换到Network选项卡，输入用户名之后点击登录，得到如下结果

![验证用户名的数据包](http://bindog.qiniudn.com/hack-hongxing/hongxin-login.png)

响应非常简单`{"exists": true}`，表示该用户存在
接着输入密码，注意勾选`Preserve log`，否则页面跳转之后就无法看到数据包了，登录成功之后得到如下数据包

![验证用户名和密码的数据包](http://bindog.qiniudn.com/hack-hongxing/hongxin-login2.png)
...
![没有代理的数据包](http://bindog.qiniudn.com/hack-hongxing/un-network-login.png)
...
![解压后的一些文件](http://bindog.qiniudn.com/hack-hongxing/hongxin-folder.png)
...
![动态获取到代理服务器地址](http://bindog.qiniudn.com/hack-hongxing/hongxin-hostinfo.png)
...
![data图片](http://bindog.qiniudn.com/hack-hongxing/hongxin-userinfo.png)
...
![破解成功图](http://bindog.qiniudn.com/hack-hongxing/hongxin-vip.png)
...
![过程图](http://bindog.qiniudn.com/hack-hongxing/hongxin-debug.png)
