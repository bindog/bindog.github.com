...
1. `shell`会立即创建7个进程
2. 每个进程的标准输入(`stdin`)和标准输出(`stdout`)将被重定向到`shell`的内部缓存中(在我的机器上每个这样的缓存大小为512 bytes，你可以用`ulimit -a`命令来查看你自己机器上这个缓存的大小)
...
![pipeline_diagram](http://i1378.photobucket.com/albums/ah103/bind0g/pipes/pipeline_diagram_zps3bbfadfc.png)
...
![pipeline_graphs](http://i1378.photobucket.com/albums/ah103/bind0g/pipes/pipeline_graphs_zpsd26d6287.png)

* 其中内存占用最大的过滤器是`cowsay`，占用了`2,830,336` bytes，因为它是用`Perl`实现的(在我的机器上仅启动`Perl`的解释器就要占用`1,126,400` bytes的内存)，内存占用最小的是`tail`，只占用了`389,120` bytes
...
* 注意到前两个进程`cat`和`grep`有大量自愿上下文切换(voluntary context switches)，这主要是由IO阻塞引起的(毕竟缓存大小只有512 bytes)。`cat`从磁盘上读取文件时必须将上下文切换到操作系统，然后将读取的内容输出到`stdout`，`grep`从管道读取数据时同样要进行上下文切换。而`ack`、`sort`、`tail`、`cut`并没有太多上下文切换的原因在于它们处理的数据量要小的多，`grep`已经替它们过滤了大量数据，实际交由`ack`处理的数据只有12行，任何一个缓存都可以装下这些数据
* `cowsay`有许多非自愿上下文切换(involuntary context switches)，不过这并不是它所处理的数据太多引起的。前面提到了`cowsay`是用`Perl`实现的，效率与其他相比是较低的，因此这里较多的非自愿上下文切换是进程时间片(time quantum )用完引起的
...
![pipeline_stderr](http://i1378.photobucket.com/albums/ah103/bind0g/pipes/pipeline_stderr_zpsfe2dac9f.png)
...
因此，我创建了一个基于`Redis`且同步的分布式管道库——[`pressure`](http://github.com/psobot/pressure),`pressure`允许你在不同进程之间建立管道，并加入了管道缓存区的概念，而且支持跨多台机器使用。通过使用`Redis`作为稳定的消息代理，所有进程间通信都可以被`Redis`接管，能保证很好的操作系统与平台无关性。(`Redis`还拥有可靠性和可复用性等诸多优点)
